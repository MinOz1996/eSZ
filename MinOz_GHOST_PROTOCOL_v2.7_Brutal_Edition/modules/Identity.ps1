...
function Apply-AegisIdentity {
    ...
    try {
        ...
        # 2c. CPU Masking (PERMANENT & Multi-Core)
        Write-AegisLog -Level "INFO" -Message "[MinOz] Masking CPU (All Cores)..."
        try {
            $cpuPath = "HKLM:\HARDWARE\DESCRIPTION\System\CentralProcessor"
            if (Test-Path $cpuPath) {
                Get-ChildItem $cpuPath | ForEach-Object {
                    ...
                    Set-ItemProperty -Path $_.PSPath -Name "Identifier" -Value "Intel64 Family 6 Model 191 Stepping 2" -Force -ErrorAction SilentlyContinue
                }
                Write-AegisLog -Level "DEBUG" -Message "[MinOz] Set CPU: $($Identity.CPU) on all cores"
            }
        }
        catch {
        ...
        # 2d. GPU Masking (Enum & Class keys)
        Write-AegisLog -Level "INFO" -Message "[MinOz] Masking GPU (Deep Layer)..."
        $gpuCount = 0
        try {
            # 1. Display Enum
            ...
            # 2. Video Controller Class
            $gpuClassPath = "HKLM:\SYSTEM\CurrentControlSet\Control\Class\{4d36e968-e325-11ce-bfc1-08002be10318}"
            if (Test-Path $gpuClassPath) {
                # Use -ErrorAction SilentlyContinue to skip protected subkeys like 'Properties'
                Get-ChildItem $gpuClassPath -ErrorAction SilentlyContinue | Where-Object { $_.PSChildName -match "^\d{4}$" } | ForEach-Object {
                    try {
                        Set-ItemProperty -Path $_.PSPath -Name "DriverDesc" -Value $Identity.GPU -Force -ErrorAction SilentlyContinue
                        Set-ItemProperty -Path $_.PSPath -Name "ProviderName" -Value "NVIDIA" -Force -ErrorAction SilentlyContinue
                        # Also set HardwareInformation.AdapterString for Task Manager
                        Set-ItemProperty -Path $_.PSPath -Name "HardwareInformation.AdapterString" -Value $Identity.GPU -Force -ErrorAction SilentlyContinue
                    } catch {}
                }
            }
            # 3. WinSAT (Task Manager often reads from here)
            $winSatPath = "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\WinSAT"
            if (-not (Test-Path $winSatPath)) { New-Item -Path $winSatPath -Force | Out-Null }
            Set-ItemProperty -Path $winSatPath -Name "PrimaryAdapterString" -Value $Identity.GPU -Force -ErrorAction SilentlyContinue

            # 4. PCI Enum (Brutal Mode - This is where Task Manager gets the root name)
            ...
            if (Test-Path $pciPath) {
                Get-ChildItem $pciPath -ErrorAction SilentlyContinue | ForEach-Object {
                    Get-ChildItem $_.PSPath -ErrorAction SilentlyContinue | ForEach-Object {
                        $deviceDesc = (Get-ItemProperty -Path $_.PSPath -Name "DeviceDesc" -ErrorAction SilentlyContinue).DeviceDesc
                        if ($deviceDesc -match "NVIDIA|AMD|Radeon|GeForce|RTX|RX|Graphics") {
                            try {
                                # Task Manager often uses the string after the semicolon in DeviceDesc
                                $newName = "Display.inf:%nvidia_dev.2204%;" + $Identity.GPU
                                Set-ItemProperty -Path $_.PSPath -Name "DeviceDesc" -Value $newName -Force -ErrorAction SilentlyContinue
                                Set-ItemProperty -Path $_.PSPath -Name "FriendlyName" -Value $Identity.GPU -Force -ErrorAction SilentlyContinue
                            } catch {}
                        }
                    }
            ...
            # 5. DirectX / DXGI Cache (Task Manager Performance Tab)
            $dxCachePath = "HKLM:\SOFTWARE\Microsoft\DirectX"
            if (Test-Path $dxCachePath) {
                Set-ItemProperty -Path $dxCachePath -Name "LastKnownDescription" -Value $Identity.GPU -Force -ErrorAction SilentlyContinue
            }

            Write-AegisLog -Level "DEBUG" -Message "[MinOz] Set GPU on $gpuCount devices: $($Identity.GPU) (Brutal Mode Active)"
        }
        catch {
        ...
        try {
            ...
            if (Test-Path $scsiPath) {
                Get-ChildItem $scsiPath -Recurse -ErrorAction SilentlyContinue | ForEach-Object {
                    try { 
                        Set-ItemProperty -Path $_.PSPath -Name "FriendlyName" -Value $Identity.DiskModel -Force -ErrorAction SilentlyContinue 
                        Set-ItemProperty -Path $_.PSPath -Name "DeviceDesc" -Value $Identity.DiskModel -Force -ErrorAction SilentlyContinue
                    } catch {}
                }
            }
            # 3. IDE Enum (For older systems or compatibility)
            $idePath = "HKLM:\SYSTEM\CurrentControlSet\Enum\IDE"
            if (Test-Path $idePath) {
                Get-ChildItem $idePath -Recurse -ErrorAction SilentlyContinue | ForEach-Object {
                    try { 
                        Set-ItemProperty -Path $_.PSPath -Name "FriendlyName" -Value $Identity.DiskModel -Force -ErrorAction SilentlyContinue 
                        Set-ItemProperty -Path $_.PSPath -Name "DeviceDesc" -Value $Identity.DiskModel -Force -ErrorAction SilentlyContinue
                    } catch {}
            ...
            # 3. Disk Service Enum
            $diskServicePath = "HKLM:\SYSTEM\CurrentControlSet\Services\disk\Enum"
            if (Test-Path $diskServicePath) {
                Get-ItemProperty -Path $diskServicePath | Get-Member -MemberType NoteProperty | ForEach-Object {
                    if ($_.Name -match "^\d+$") {
                        # We can't easily change the value here as it points to Enum paths, but we've changed the Enum paths above
                    }
                }
            }
        } catch {
            Write-AegisLog -Level "WARN" -Message "[MinOz] Failed to mask Disk: $($_.Exception.Message)"
        }

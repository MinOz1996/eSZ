...
function Get-CurrentSystemIdentity {
    ...
    $components = @(
        ...
        @{
            Name       = "VolumeId"
            ScriptBlock = {
                # Get-Volume ไม่มี SerialNumber - ใช้ WMI แทน
                try {
                    $vol = Get-WmiObject Win32_LogicalDisk -Filter "DriveType=3" -ErrorAction Stop | Select-Object -First 1
                    if ($vol) { return $vol.VolumeSerialNumber }
                } catch {}
                "N/A"
            }
        }
    ...
    if (Test-Path $script:BACKUP_DIR) {
        ...
        if ($existingBackups -and $existingBackups.Count -gt 0) {
            ...
            switch ($choice) {
                ...
                default {
                    # Empty Enter หรือ invalid = SKIP (ปลอดภัยที่สุด)
                    Write-Host "[INFO] Skipping backup - keeping existing ORIGINAL backup" -ForegroundColor Green
                    Write-AegisLog -Level "INFO" -Message "Backup skipped (default) - existing backup preserved"
                    return
                }
            }
        }
...
function Restore-AegisSystem {
    ...
    if (Test-Path $script:BACKUP_DIR) {
        ...
        if ($null -eq $regFiles -or $regFiles.Count -eq 0) {
            ...
            Write-Host "  |  NOTICE: No backup found                           |" -ForegroundColor Yellow
            Write-Host "  |                                                    |" -ForegroundColor DarkMagenta
            Write-Host "  |  Your system has not been spoofed yet.             |" -ForegroundColor White
            Write-Host "  |  Nothing to restore - your values are original.    |" -ForegroundColor White
            Write-Host "  |                                                    |" -ForegroundColor DarkMagenta
            Write-Host "  |  To use this tool:                                 |" -ForegroundColor Cyan
            Write-Host "  |    [1] GHOST PROTOCOL FULL  - Run spoof first      |" -ForegroundColor Cyan
            Write-Host "  |    [2] STANDARD PROTECTION  - Lighter spoof mode   |" -ForegroundColor Cyan
            Write-Host "  |                                                    |" -ForegroundColor DarkMagenta
            Write-Host "  +----------------------------------------------------+" -ForegroundColor DarkMagenta
            Write-Host ""
            return
...
#region Aegis State Management

<#
.SYNOPSIS
    Saves a specific state variable to the Aegis state file.
.PARAMETER Key
...
function Set-AegisState {
    ...
    if (Test-Path $script:AEGIS_STATE_FILE) {
        try {
            $json = Get-Content -Path $script:AEGIS_STATE_FILE -Raw -ErrorAction Stop
            $parsed = $json | ConvertFrom-Json -ErrorAction Stop
            # Safe convert: ถ้า -as [hashtable] คืน $null ให้สร้างใหม่
            if ($parsed -ne $null) {
                $state = ConvertTo-Hashtable -InputObject $parsed
                if ($null -eq $state) { $state = @{} }
            }
        } catch {
            Write-AegisLog -Level "WARN" -Message "State file unreadable, starting fresh: $($_.Exception.Message)"
            $state = @{}
        }
    }
    ...
    try {
        ...
        Set-Content -Path $script:AEGIS_STATE_FILE -Value $json -Encoding UTF8 -Force -ErrorAction SilentlyContinue
        Write-AegisLog -Level "DEBUG" -Message "Saved Aegis state: ${Key} = ${Value}"
    } catch {
        Write-AegisLog -Level "ERROR" -Message "Failed to save Aegis state for ${Key}: $($_.Exception.Message)"
    }
}

<#
.SYNOPSIS
    Retrieves a specific state variable from the Aegis state file.
.PARAMETER Key
    The name of the state variable.
.PARAMETER Default
...
function Get-AegisState {
    ...
    try {
        $json = Get-Content -Path $script:AEGIS_STATE_FILE -Raw -ErrorAction Stop
        $parsed = $json | ConvertFrom-Json -ErrorAction Stop
        $state = if ($parsed) { ConvertTo-Hashtable -InputObject $parsed } else { @{} }
        if ($null -eq $state) { $state = @{} }
        if ($state.ContainsKey($Key)) {
            Write-AegisLog -Level "DEBUG" -Message "Retrieved Aegis state: ${Key} = $($state[$Key])"
            return $state[$Key]
        }
    } catch {
        Write-AegisLog -Level "WARN" -Message "Failed to load Aegis state file for ${Key}, returning default: $($_.Exception.Message)"
    }

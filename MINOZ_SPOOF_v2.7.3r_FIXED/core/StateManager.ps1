...
function Get-CurrentSystemIdentity { 
    $identity = @{}
    try {
        $identity.ComputerName = (Get-ItemProperty "HKLM:\SYSTEM\CurrentControlSet\Control\ComputerName\ComputerName").ComputerName
        $identity.MachineGuid = (Get-ItemProperty "HKLM:\SOFTWARE\Microsoft\Cryptography").MachineGuid
        $identity.Manufacturer = (Get-ItemProperty "HKLM:\SYSTEM\CurrentControlSet\Control\SystemInformation").SystemManufacturer
        $identity.Product = (Get-ItemProperty "HKLM:\SYSTEM\CurrentControlSet\Control\SystemInformation").SystemProductName
        $identity.CPU = (Get-ItemProperty "HKLM:\HARDWARE\DESCRIPTION\System\CentralProcessor\0").ProcessorNameString
        
        # Add Network Info
        $adapters = Get-WmiObject Win32_NetworkAdapterConfiguration -Filter "IPEnabled = True"
        if ($adapters) {
            $identity.MacAddress = $adapters[0].MACAddress
        }
    } catch {
        Write-AegisLog -Level "WARN" -Message "Failed to retrieve full system identity: $($_.Exception.Message)"
    }
    return $identity
}
...
function Restore-AegisSystem {
    ...
    # Safely call restore functions if they exist
    if (Get-Command Remove-GHOSTProtocol -ErrorAction SilentlyContinue) {
        try { Remove-GHOSTProtocol } catch { Write-AegisLog -Level "ERROR" -Message "Remove-GHOSTProtocol failed: $($_.Exception.Message)" }
    } else {
        Write-AegisLog -Level "WARN" -Message "Remove-GHOSTProtocol function not found."
    }

    if (Get-Command Restore-OriginalIdentity -ErrorAction SilentlyContinue) {
        try { Restore-OriginalIdentity } catch { Write-AegisLog -Level "ERROR" -Message "Restore-OriginalIdentity failed: $($_.Exception.Message)" }
    } else {
        Write-AegisLog -Level "WARN" -Message "Restore-OriginalIdentity function not found."
    }

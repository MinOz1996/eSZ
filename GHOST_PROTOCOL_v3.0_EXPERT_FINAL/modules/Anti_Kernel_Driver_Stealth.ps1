...
function Invoke-KernelObliteration { Invoke-AntiKernelDriverStealth }
function Invoke-AntiKernelDriverStealth {
    ...
    Write-AegisLog -Level "INFO" -Message "[MinOz] Invoking Anti-Kernel-Debugger & Driver Stealth Module..."

    try {
        # --- 1. Disable Kernel Debugging (Registry Modifications) ---
        if (-not $CleanOnly) {
            Write-AegisLog -Level "DEBUG" -Message "[MinOz] Attempting to disable kernel debugging via registry..."
            try {
                ...
                Write-AegisLog -Level "INFO" -Message "[MinOz] Kernel debugging settings modified. Debugger launch attempts may be blocked."
            } catch {
                Write-AegisLog -Level "WARN" -Message "[MinOz] Failed to modify kernel debugging settings: $($_.Exception.Message)"
            }
        }

        # --- 2. Driver Stealth & Cleanup (Complementary to Kernel_Trace_Obliteration) ---
        Write-AegisLog -Level "DEBUG" -Message "[MinOz] Performing driver-related stealth cleanup..."
        try {
                ...
                foreach ($pattern in $suspiciousDriverPatterns) {
                    Get-ChildItem -Path $driverStorePath -Filter $pattern -Recurse -ErrorAction SilentlyContinue | ForEach-Object {
                        try {
                            ...
                            Write-AegisLog -Level "DEBUG" -Message "[MinOz] Removed suspicious driver store entry: $($_.FullName)"
                        } catch {
                            Write-AegisLog -Level "WARN" -Message "[MinOz] Failed to remove driver store entry $($_.FullName): $($_.Exception.Message)"
                        }
                    }
                }
            }
                ...
                Get-ChildItem $pnpKeyPath -Recurse -ErrorAction SilentlyContinue | ForEach-Object {
                    if ($_.PSPath -match ".*\Device Parameters$") {
                        try {
                            ...
                            Write-AegisLog -Level "DEBUG" -Message "[MinOz] Cleaned PnP history for $($_.PSPath)"
                        } catch {
                            Write-AegisLog -Level "WARN" -Message "[MinOz] Failed to clean PnP history for $($_.PSPath): $($_.Exception.Message)"
                        }
                    }
                }
            }

            Write-AegisLog -Level "INFO" -Message "[MinOz] Driver-related stealth cleanup completed."
        } catch {
            Write-AegisLog -Level "ERROR" -Message "[MinOz] Driver stealth cleanup failed: $($_.Exception.Message)"
        }

        Write-AegisLog -Level "INFO" -Message "[MinOz] Anti-Kernel-Debugger & Driver Stealth Module completed."
    } catch {
        Write-AegisLog -Level "ERROR" -Message "[MinOz] Anti-Kernel-Debugger & Driver Stealth Module failed: $($_.Exception.Message)"
...
function Remove-AntiKernelDriverStealth {
    [CmdletBinding()]
    param()
    Write-AegisLog -Level "INFO" -Message "[MinOz] Removing Anti-Kernel-Debugger & Driver Stealth modifications (relying on system restore)."
    ...
    try {
        ...
        foreach ($debugger in $debuggers) {
            ...
            if (Test-Path $keyPath) {
                ...
                Write-AegisLog -Level "DEBUG" -Message "[MinOz] Removed Image File Execution Options for $debugger."
            }
        }
        ...
        Write-AegisLog -Level "INFO" -Message "[MinOz] Anti-Kernel-Debugger & Driver Stealth removal completed."
    } catch {
        Write-AegisLog -Level "ERROR" -Message "[MinOz] Anti-Kernel-Debugger & Driver Stealth removal failed: $($_.Exception.Message)"
        # Non-critical: continue execution
    }
}

...
function Invoke-WMIMutation {
    ...
    if ($null -eq $Identity) {
        if (Get-Command New-EliteIdentity -ErrorAction SilentlyContinue) {
            $Identity = New-EliteIdentity
        } else {
            Write-AegisLog -Level "ERROR" -Message "[GHOST] Identity generator not found. Aborting."
            return
        }
    }
...
"@
        ...
        Write-AegisLog -Level "WARN" -Message "[GHOST] WMI Mutation partial (user-mode limit): $($_.Exception.Message)"
        #  throw - WMI mutation failure  user-mode
    }
}

...
function Invoke-AegisCleaner {
    ...
    try {
        ...
        # WMI Repository (DISABLED - causes Task Manager to close)
        # Remove-Item "C:\Windows\System32\wbem\Repository" -Recurse -Force -ErrorAction SilentlyContinue
        # net start winmgmt /y 2>$null
        Write-AegisLog -Level "INFO" -Message "[MinOz] WMI Repository clean skipped (prevents Task Manager closure)."
    } catch {}

...
function Set-AegisState {
    ...
    if (Test-Path $script:AEGIS_STATE_FILE) {
        try {
            $state = (Get-Content -Path $script:AEGIS_STATE_FILE -Raw | ConvertFrom-Json) -as [hashtable]
        } catch {
            Write-AegisLog -Level "WARN" -Message "Failed to load Aegis state file, creating new one: $($_.Exception.Message)"
        }
    }

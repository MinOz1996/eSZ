...
#region JSON Hashtable Conversion

<#
.SYNOPSIS
    Converts PSCustomObject (from ConvertFrom-Json) to a proper Hashtable recursively.
    CRITICAL: ConvertFrom-Json returns PSCustomObject not Hashtable in PS5.1
#>
        ...
        if ($InputObject -is [System.Management.Automation.PSCustomObject]) {
            ...
            foreach ($prop in $InputObject.PSObject.Properties) {
                ...
                if ($val -is [System.Management.Automation.PSCustomObject]) {
                    $result[$prop.Name] = ConvertTo-Hashtable -InputObject $val
                } elseif ($val -is [System.Object[]]) {
                    $result[$prop.Name] = $val | ForEach-Object { ConvertTo-Hashtable -InputObject $_ }
                } else {

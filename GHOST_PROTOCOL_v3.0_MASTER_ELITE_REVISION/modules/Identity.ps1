...
function Apply-AegisIdentity {
    ...
    # 1. Registry Backup before change
    $backupDir = Join-Path $script:AegisRoot "backup"
    if (!(Test-Path $backupDir)) { New-Item -Path $backupDir -ItemType Directory -Force }
    reg export "HKLM\HARDWARE\DESCRIPTION\System\BIOS" (Join-Path $backupDir "bios.reg") /y | Out-Null
    reg export "HKLM\SYSTEM\CurrentControlSet\Control\SystemInformation" (Join-Path $backupDir "sysinfo.reg") /y | Out-Null
    reg export "HKLM\SOFTWARE\Microsoft\Cryptography" (Join-Path $backupDir "crypto.reg") /y | Out-Null
    ...
    Set-ItemProperty -Path "HKLM:\SYSTEM\CurrentControlSet\Control\ComputerName\ComputerName" -Name "ComputerName" -Value $Identity.ComputerName -Force -ErrorAction SilentlyContinue
    
    # 3. Hardware & BIOS (Full Mutation)
    $paths = @("HKLM:\HARDWARE\DESCRIPTION\System\BIOS", "HKLM:\SYSTEM\CurrentControlSet\Control\SystemInformation")
